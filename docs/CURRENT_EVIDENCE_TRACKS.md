...
| EU Data Portal source audit D-001 | European Data Portal (data.europa.eu) | `PASSED_UNDER_PROTOCOL` | The European Data Portal landing page passed the source-audit gate for page reachability, dataset catalog link, hub search API link and copyright notice link only. |
...
program eligibility categories under the frozen manual checklist.
```
...
implying program-sponsor affiliation. The card is single-operator evidence only and is
