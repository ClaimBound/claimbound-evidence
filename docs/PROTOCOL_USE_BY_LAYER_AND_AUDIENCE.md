...
| Evidence-card layer | validated JSON/SVG card and sanitized report | What happened under the frozen protocol? | A completed source audit, reproduction, regression, parity, benchmark or checklist run has one exact status. | Whole-project correctness, certification, deployment readiness or broad model superiority. |
...
| Public procurement or funding review | Validated card plus registry/reporting links; v2 when several related promises or milestones must be tracked. v3 when the reviewer needs a map. | Reviewers need status and limitations, not narrative success language. |
...
| Funding reviewers and program evaluators | Narrative milestone reports without source/status/limitation trace. | Track promised milestones, proof surfaces, stop rules, negative outcomes and closure. | Give reviewers a compact project map across claims, dependencies, tombstones and validated cards. | Which milestone claim was checked, what source was used, what happened and what cannot be claimed. | Treating a demo, scaffold or future plan as evidence. |
