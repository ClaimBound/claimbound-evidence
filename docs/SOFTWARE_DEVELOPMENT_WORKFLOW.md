...
manual QA, code review, release engineering, maintainer judgment or ordinary
project quality practices. ClaimBound records and explains selected checks; it
does not become the checks.
...
one written protocol. It does not mean the whole project is correct, complete,
stable or deployment-ready.

## When It Fits

Use ClaimBound for complex, risky, public, assistant-assisted or
regression-sensitive software changes. Do not use a full track for every typo,
formatting edit, small comment or disposable prototype.

Good fit examples:

- public pull requests that change behavior users rely on;
- regression-sensitive fixes;
- assistant-assisted patches that need deterministic verification;
- compatibility, parity or behavior checks with fixed fixtures;
...
| Regression risk | Records commands, fixtures, logs, environment notes and limitations. |
| Assistant-assisted patches | Separates generated code from deterministic runner or checklist results. |
...
| `The issue is fixed.` | `The documented reproduction path completes on the declared local environment.` |
...
| `The assistant patch is good.` | `The assistant-assisted patch passes the fixed runner, respects the declared file boundary and leaves the declared regression checks green.` |
...
### Regression-sensitive fix

```text
Claim 1: The project builds on the declared local environment.
Claim 2: The existing test suite remains green.
Claim 3: The documented reproduction path now completes.
Claim 4: The sanitized log contains no new error marker for the tested path.
...
Claim 4: The result comes from the command, test or validator, not from model opinion.
```
...
### Assistant-assisted patch

```text
Claim 1: The assistant-assisted change is limited to declared files or modules.
Claim 2: The generated code passes the fixed runner or checklist.
Claim 3: The patch keeps existing checks unless the protocol explicitly changes them.
Claim 4: A human maintainer approves final status and boundary wording.
```

## What ClaimBound Does Not Replace

ClaimBound can cite or summarize ordinary checks, but it must not replace them.
...
- claiming deployment readiness from a local-only check;
- turning subjective judgment into a fake objective result after seeing output;
- using a card instead of tests, CI or review;
...
| --- | --- | --- |
| Strong fit | regression-sensitive behavior, public API changes, migrations, assistant-assisted patches, cross-platform compatibility, R&D proof tracks | Use a full ClaimBound track with claims, commands/checklist, artifacts and limitations. |
...
| Not appropriate | broad correctness, deployment readiness, subjective quality claims | Split into narrow claims or use ordinary engineering/governance processes instead. |
