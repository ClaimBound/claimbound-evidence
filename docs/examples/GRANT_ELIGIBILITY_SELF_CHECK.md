# Grant Eligibility Self-Check Example

This example shows how ClaimBound can be used by an applicant to check their own
public project against public grant eligibility criteria without turning the
result into a funding decision claim.

It is intentionally narrow. It does not claim that a grant maker should fund the
...
Grant applications often contain broad statements:
...
The evidence card does not claim grant approval, reviewer endorsement or private
...
A grant self-check is acceptable only if it stays inside these limits:

- Use public call pages, public repository facts and public project documents.
- Do not imply endorsement by the grant maker.
...
| The self-check is independent from the grant maker. | Evidence card boundary text. | Must explicitly say "not an endorsement, not a funding decision". |
...
```json
{
  "claim_id": "CLAIMBOUND-GRANT-ELIGIBILITY-SELF-CHECK-D001",
  "claim": "A public applicant-side self-check can map public grant eligibility rules to narrow ClaimBound evidence claims without claiming grant approval.",
