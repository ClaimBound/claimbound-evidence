...
"""Render ClaimBound evidence-card SVG from JSON."""
...
from claimbound_evidence.card_svg_styles import (
    HEIGHT,
    WIDTH,
    gradient_defs,
    reproduction_gradient,
    result_gradient,
    style_attr,
    validity_gradient,
)
...
def render_svg(card_path: Path) -> str:
...
def render_all_cards(cards_dir: Path) -> list[Path]:
    rendered: list[Path] = []
    for card_path in sorted(cards_dir.glob("CLAIMBOUND-*.json")):
        svg_path = card_path.with_suffix(".svg")
        svg_path.write_text(render_svg(card_path), encoding="utf-8")
...
def _visual_values(card: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "card_validity_level": str(card.get("card_validity_level") or _default_card_validity(card)),
...
def _render_card(values: dict[str, Any]) -> str:
    out = [
        ...
        *gradient_defs(),
        ...
        f'  <text x="70" y="93" {style_attr("title")}>ClaimBound Evidence Card</text>',
        f'  <text x="73" y="129" {style_attr("subtitle")}>Protocol-bound, reproducible evidence summary</text>',
        *(_logo_lockup(1415, 52, 0.33)),
    ]

    out.extend(_chip(70, 193, 430, "Result status", values["result_status"], result_gradient(values["result_status"])))
    out.extend(_chip(520, 193, 370, "Validity", values["card_validity_level"], validity_gradient(values["card_validity_level"])))
    out.extend(_chip(910, 193, 480, "Reproduction", values["reproduction_level"], reproduction_gradient(values["reproduction_level"])))
    out.extend(_chip(1410, 193, 520, "Record type", values["record_type"], "blueGrad"))
    ...
    out.extend(_wide_note(70, 938, 1860, "Claim boundary", values["claim_boundary"], values["known_limitations"]))
    out.append("</svg>")
    return "\n".join(out) + "\n"
...
def _chip(x: int, y: int, w: int, label: str, value: str, gradient_id: str) -> list[str]:
    lines = [
        f'  <text x="{x}" y="{y - 18}" {style_attr("chipLabel")}>{_e(label)}</text>',
...
def _claim_box(x: int, y: int, w: int, claim: str) -> list[str]:
    lines = [
        ...
        f'  <text x="{x + 26}" y="{y + 31}" {style_attr("claimLabel")}>Allowed narrow claim</text>',
    ]
...
def _field_box(
    ...
    *,
    ...
    text_style = "mono" if mono else "fieldText"
    ...
    lines = [
        ...
        f'  <text x="{x + 22}" y="{y + 29}" {style_attr("fieldLabel")}>{_e(label)}</text>',
    ]
    lines.extend(_text_lines(x + 22, y + 58, wrapped, text_style, line_height))
    return lines
...
def _wide_note(
    ...
    lines = [
        ...
        f'  <text x="{x + 22}" y="{y + 28}" {style_attr("fieldLabel")}>{_e(label)}</text>',
    ]
...
def _logo_lockup(x: int, y: int, scale: float) -> list[str]:
    return [
        ...
        '        <path d="M130 507V714"/><path d="M162 490V719"/><path d="M199 491V719"/>',
        '        <path d="M230 471V726"/><path d="M276 521V690"/><path d="M327 508V650"/>',
        '        <path d="M128 520H277"/><path d="M128 595H327"/><path d="M128 672H355"/>',
        "      </g>",
...
def _text_lines(x: int, y: int, lines: list[str], style_name: str, line_height: int) -> list[str]:
    attrs = style_attr(style_name)
    return [f'  <text x="{x}" y="{y + idx * line_height}" {attrs}>{_e(line)}</text>' for idx, line in enumerate(lines)]


def _wrap(value: str, width: int, *, max_lines: int) -> list[str]:
    paragraphs = [" ".join(part.split()) for part in str(value or "").splitlines() if part.strip()]
    if not paragraphs:
        return ["n/a"]
    lines: list[str] = []
    for paragraph in paragraphs:
        lines.extend(textwrap.wrap(paragraph, width=max(8, width), break_long_words=True, break_on_hyphens=False))
