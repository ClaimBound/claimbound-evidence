...
def build_scaffold(request: ScaffoldRequest, repo_root: Path) -> list[Path]:
    ...
    tree_overlay_path = repo_root / "docs" / "track_families" / f"{protocol_id}_TREE.json"
    ...
    tree_overlay_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    _write_json(tree_overlay_path, _render_tree_overlay(request, protocol_id))
...
- optional protocol v3 tree overlay claim nodes and branch block rules;
...
uv run claimbound validate-tree docs/track_families/{protocol_id}_TREE.json
```
...
docs/track_families/{protocol_id}_TREE.json
...
I did not treat diagnostic screening or flow routing as a proof or deployment claim.
...
def _render_draft_card(request: ScaffoldRequest, protocol_id: str, created_at: str) -> dict[str, object]:
    return {
        ...
        "claim_boundary": "Draft only. No result is claimed until a real run produces validated artifacts under the frozen protocol.",
...
def _render_family_ledger(request: ScaffoldRequest, protocol_id: str) -> dict[str, object]:
    return {
        ...
        "non_overlap_boundary": "Describe what makes this family new relative to previous tracks. Do not reuse a failed source, label, mechanics or gate as a new proof.",
        "proof_surface": {
        ...
        "blocked_claim_flags": ["broad_model_superiority", "deployment_readiness", "correctness_outside_protocol"],
        "context_budget": {"max_context_lines": 80, "capsule_required": True, "capsule_note": "Future related tracks should read this ledger before full reports."},
        "claim_scope": {
            "allowed": [f"Draft family planning for {request.track_type} evidence under {protocol_id}.", "Diagnostic claims may identify candidates only."],
            "forbidden": ["No broad model superiority claim.", "No deployment-readiness claim.", "No correctness claim outside the protocol boundary."],
        },
        "track_budget": {"max_proof_tracks_per_hypothesis": 3, "budget_note": "After the budget is exhausted, add a closure decision or register a genuinely non-overlapping hypothesis family."},
        "stop_rules": [
        ...
        "claim_list": [
            {
                ...
                "evidence_required": ["source name", "source URL", "access date", "rights or attribution note"],
                "acceptance_gate": "Source boundary is recorded before outcome scoring.",
                "forbidden_inference": ["source availability alone does not prove the empirical claim"],
                "depends_on": [],
            ...
            },
            {
                ...
                "evidence_required": ["frozen diagnostic labels or checks", "sanitized diagnostic summary", "explicit no-proof boundary"],
                "acceptance_gate": "Candidate or blocker is recorded without a positive result claim.",
                "forbidden_inference": ["diagnostic output is not a proof claim", "diagnostic output is not a deployment claim"],
            ...
            },
            {
                ...
                "evidence_required": ["frozen candidate", "frozen labels or target", "baselines and controls", "acceptance gate", "sanitized result summary"],
                "acceptance_gate": "The candidate passes only if the frozen gate passes.",
                "forbidden_inference": ["one passed proof does not support a broader family claim", "negative or blocked outcomes must not be renamed as success"],
        ...
        "frontier": {"current_frontier": [f"{protocol_id}-T001"], "consumed_tombstones": [], "tombstone_required_before_descendants": True},
    }


def _render_tree_overlay(request: ScaffoldRequest, protocol_id: str) -> dict[str, object]:
    return {
        "tree_id": f"{protocol_id}_TREE",
        "protocol_version": "claimbound-tree-v3",
        "tree_status": "DRAFT",
        "root_family_id": f"{protocol_id}_FAMILY",
        "claim_nodes": [
            {
                "claim_id": f"{protocol_id}-C001",
                "claim_kind": "iron_claim",
                "status": "DRAFT",
                "claim_text": "Official or public source boundary is identified before outcome scoring.",
                "evidence_required": ["source name", "source URL", "access date", "rights or attribution note"],
                "acceptance_gate": "Source boundary is recorded before outcome scoring.",
                "forbidden_inference": ["source availability alone does not prove the empirical claim"],
                "proof_surface_hash": "NOT_COMPUTED_UNTIL_FREEZE",
            },
            {
                ...
                "status": "DRAFT",
                "claim_text": "Diagnostic screening may identify candidate signals or source gaps without proof status.",
                "evidence_required": ["diagnostic summary", "explicit no-proof boundary"],
                "acceptance_gate": "Candidate or blocker is recorded without a positive result claim.",
                "forbidden_inference": ["diagnostic output is not a proof, deployment or production claim"],
            },
        ],
        "track_nodes": [
            {
        ...
        "tombstones": [],
        "badge_counts": {"iron_claims": 1, "flow_claims": 1, "tombstones": 0},
        "branch_block_rules": [
            "A stopped branch must publish a tombstone before descendants can be treated as runnable.",
            "A diagnostic or flow claim must not be cited as proof, deployment readiness or production readiness.",
        ],
    }
...
def _render_source_probe(request: ScaffoldRequest, protocol_id: str, created_at: str) -> dict[str, object]:
    ...
    return {
        ...
        "claim_boundary": "Static source probe scaffold only. This summary does not prove source access, source rights, coverage or result status.",
