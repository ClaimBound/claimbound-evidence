...
"""Preview and publish ESA factory issues #133 through #135."""
...
from esa_factory_data import BATCH_SPECS, load_matrix
...
def _paths(issue_number: int) -> dict[str, Path]:
    return {
        ...
        "summary": REPO_ROOT
        / "artifacts"
        / f"esa_issue_{issue_number}_batch_summary.json",
        "result": (
            REPO_ROOT
            / "docs"
            / "manual_audit"
            / f"ESA-ISSUE-{issue_number}"
            / "README.md"
        ),
    }
...
def _fetch_source(url: str) -> tuple[bytes | None, str | None]:
...
def _run_preview(issue_number: int, run_root_arg: str | None, quiet: bool) -> int:
    ...
    run_root = (
        ...
        if run_root_arg
        else Path.home() / "claimbound_runs" / f"ESA_ISSUE_{issue_number}_{timestamp}"
    )
    ...
    for source_url, source_cards in cards_by_url.items():
        ...
        payload, error = _fetch_source(source_url)
        if payload is None:
    ...
    preview = {
        ...
        "matrix_source": "embedded frozen matrix; exported during publish",
        ...
        "claim_boundary": (
            "This local preview evaluates exactly 100 frozen narrow claims "
            "against five official ESA mission pages. Raw HTML remains outside "
            "the repository."
        ),
        "source_records": [
...
def _build_card(
    *,
    ...
    if status == "PASSED_UNDER_PROTOCOL":
        claim_boundary = (
            "This card verifies only that the official ESA mission page contained "
        ...
        claim_boundary = (
            "This card records that the official ESA mission page did not contain "
        ...
        claim_boundary = (
            "This card records that the official ESA source could not be retrieved "
            "under the frozen local protocol for this narrow statement: "
            f"{result['claim']} This is a blocked-source result, not a scientific "
            "conclusion."
        )

    summary_path = f"artifacts/esa_issue_{issue_number}_batch_summary.json"
    card: dict[str, Any] = {
        ...
        "baseline_control_summary": (
            "Frozen all-pattern source-boundary gate against one official ESA "
            "mission page; no non-ESA source entered the gate; missing patterns "
        ...
        "manual_review": (
            ...
            "100-card preview before publication"
        ),
        ...
        "source_rights_note": (
            "Official ESA public web page. Raw HTML is kept only in the local "
...
def _run_publish(
    ...
    summary = {
        ...
        "raw_payload_committed": False,
        "claim_boundary": (
            "This batch report records 100 frozen source-boundary evaluations "
            "against five official ESA mission pages. It does not certify mission "
    ...
    temp_matrix = temp_root / "docs" / "esa" / paths["matrix"].name
    ...
    result_lines = [
        f"# ESA issue #{issue_number}: 100-card source-boundary batch",
        "",
    ...
    temp_result = (
        temp_root / "docs" / "manual_audit" / f"ESA-ISSUE-{issue_number}" / "README.md"
    )
    temp_result.parent.mkdir(parents=True, exist_ok=True)
    temp_result.write_text("\n".join(result_lines), encoding="utf-8")

    for _card, card_path, svg_path in new_cards:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--issue", type=int, choices=sorted(BATCH_SPECS), required=True)
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("check", help="Validate the frozen 100-card matrix.")

    preview = commands.add_parser(
        "preview", help="Fetch five ESA pages and write a local-only preview."
    )
