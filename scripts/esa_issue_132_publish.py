...
"""Publication phase for the ESA issue #132 local batch runner."""
...
from esa_issue_132_common import (
    CARDS_DIR,
    MATRIX_EXPORT_PATH,
    REGISTRY_PATH,
    REPO_ROOT,
...
from esa_issue_132_preview import load_preview
...
def build_card(
    *,
    ...
    card: dict[str, Any] = {
        ...
        "ai_assistance": (
            "AI-assisted drafting of the frozen issue #132 claim matrix; "
            "deterministic local regex gates selected the status; the "
            "operator reviews the preview before publication."
        ),
        "baseline_control_summary": (
            "Frozen all-pattern source-boundary gate against one official "
            "ESA mission page; no non-ESA source entered the gate; missing "
            "patterns produce INSUFFICIENT_COVERAGE rather than a forced pass."
        ),
        ...
        "manual_review": "operator confirmed review of the frozen 100-card preview before publication",
        ...
        "runner_command": (
            ...
            "--preview <local-preview.json> --operator <handle> --confirm-reviewed"
        ),
        "sanitized_report_path": "artifacts/esa_issue_132_batch_summary.json",
        "sanitized_report_sha256": summary_sha256,
        "source_rights_note": (
            "Official ESA public web page. Raw HTML is kept only in the local run root and is not committed."
        ),
        "verification_count": 1,
        "verification_level": "SINGLE_OPERATOR",
    }
    if status == "BLOCKED_SOURCE":
        card["block_reason"] = str(result.get("block_reason") or "official source unavailable")
    return card
...
def run_publish(*, preview_arg: str, operator: str, confirm_reviewed: bool) -> int:
    ...
    matrix_by_id = {card["protocol_id"]: card for card in matrix["cards"]}
    results_by_id = {result["protocol_id"]: result for result in preview["results"]}
    ...
    existing_sequences = [int(entry["registry_sequence"]) for entry in registry_cards]
    ...
    for protocol_id in sorted(results_by_id):
        if protocol_id in existing_by_protocol:
            planned_sequences[protocol_id] = int(existing_by_protocol[protocol_id]["registry_sequence"])
        else:
    ...
    for protocol_id in sorted(results_by_id):
        ...
        if existing is not None:
            ...
            if existing_card.get("official_source_url") != result["official_source_url"]:
                raise SystemExit(f"{protocol_id}: existing source URL differs from preview")
            if existing_card.get("result_status") != result["result_status"]:
                raise SystemExit(
                    f"{protocol_id}: existing status {existing_card.get('result_status')} differs from preview {result['result_status']}"
                )
    ...
    summary = {
        ...
        "raw_payload_committed": False,
        "claim_boundary": (
            "This batch report records 100 frozen source-boundary evaluations against five official ESA mission pages. It does not certify mission performance, data quality, safety or operational suitability."
        ),
    ...
    for protocol_id in sorted(results_by_id):
        ...
        if violations:
            ...
            raise SystemExit(f"{protocol_id}: generated card is invalid: {joined}")
        ...
        if card_path.exists() or svg_path.exists():
            raise SystemExit(
                f"{protocol_id}: target evidence file exists but protocol is absent from registry"
            )
    ...
    for card, card_path, _svg_path in new_cards:
        updated_registry_cards.append(registry_entry(card, card_path))

    updated_registry_cards = sorted(updated_registry_cards, key=lambda entry: str(entry["evidence_id"]))
    registry["cards"] = updated_registry_cards
    registry["card_count"] = len(updated_registry_cards)
    registry["statistics"] = registry_statistics(updated_registry_cards)

    temp_registry_path = temp_root / "docs" / "registry" / "evidence_index.json"
    temp_summary_path = temp_root / "artifacts" / SUMMARY_PATH.name
    temp_matrix_path = temp_root / "docs" / "esa" / "issue_132_claim_matrix.json"
    write_json(temp_registry_path, registry)
    temp_summary_path.parent.mkdir(parents=True, exist_ok=True)
    temp_summary_path.write_bytes(summary_payload)
    temp_matrix_path.parent.mkdir(parents=True, exist_ok=True)
    temp_matrix_path.write_bytes(matrix_payload)
    ...
    temp_result_readme = temp_root / "docs" / "manual_audit" / "ESA-ISSUE-132" / "README.md"
    temp_result_readme.parent.mkdir(parents=True, exist_ok=True)
    ...
    RUNBOOK_RESULT_PATH.parent.mkdir(parents=True, exist_ok=True)
    temp_result_readme.replace(RUNBOOK_RESULT_PATH)
    MATRIX_EXPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    temp_matrix_path.replace(MATRIX_EXPORT_PATH)

    print(f"created_cards={len(new_cards)}")
    print(f"existing_target_cards={100 - len(new_cards)}")
    print("target_cards_total=100")
    print(f"registry_card_count={registry['card_count']}")
    for status, count in counts.items():
        print(f"{status}={count}")
    print(f"batch_summary={SUMMARY_PATH.relative_to(REPO_ROOT)}")
