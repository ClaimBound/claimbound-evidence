...
"""Preview phase for the ESA issue #132 local batch runner."""
...
def evaluate_card(card: dict[str, Any], source: dict[str, Any]) -> dict[str, Any]:
...
def run_preview(*, run_root_arg: str | None, quiet: bool) -> int:
    ...
    run_root = (
        ...
        if run_root_arg
        else Path.home() / "claimbound_runs" / f"ESA_ISSUE_132_{timestamp}"
    )
    ...
    results = [
        evaluate_card(card, source_records[str(card["source_url"])])
        for card in matrix["cards"]
    ]
    counts = dict(
        sorted(
            Counter(result["result_status"] for result in results).items()
        )
    )
...
def load_preview(path: Path) -> dict[str, Any]:
    ...
    if preview.get("issue_number") != 132:
        raise ValueError("preview is not for issue #132")
